...
| 路線 provider 不可用 | 有起終點、provider 失敗 | 不造距離／時間／污染分數，顯示資料不足與外部地圖交接 |
| 路線 provider 可用 | 有起終點、Mapbox Directions 回應 | 可顯示 provider 給的距離／預估時間與路線預覽；仍不可聲稱最低污染、最安全、街道級 AQI 或導航保證 |
| 活動地區不一致 | 活動說台中、選定區域為高雄 | 不使用錯誤區域環境，要求先更新選定區域 |
| 未來活動 | 明天／下週的活動 | 不把當前 AQI 當預報，標示 partial 並提醒活動前重查 |
...
- [教育部「高級中等以下學校及幼兒園因應空氣品質惡化處理措施暨緊急應變作業流程」](https://edu.law.moe.gov.tw/LawContent.aspx?id=GL001315&media=print)（2023-12-18 修正）
- [環境部 AQI 指標與活動建議](https://airtw.moenv.gov.tw/CHT/Information/Standard/AirQualityIndicatorNew.aspx)
