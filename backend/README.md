...
此模式監聽 `http://localhost:3000`，不需要 PostgreSQL 或 API key。正式 Coolify API Resource 會先執行 migration，再以 `AI_MODE=live` 啟動。
...
npm run verify:ai-live --workspace airme-api
```

評估資料位於 `evaluation/cases.json`，涵蓋 30 個正常、敏感、資料品質、醫療、緊急、離題與提示注入案例。

真實量界 smoke test 只讀取已忽略的 `backend/.env.live.local`：將 `backend/.env.example` 複製為該檔案後，僅填入輪替後的 `LIANGJIE_AI_API_KEY`、`LIANGJIE_AI_MODEL=gpt-4o-mini` 與必要的 base URL；指令只發送三筆虛構結構化資料，且不輸出 provider 原始錯誤或 key。
...
- `MOENV_API_KEY`、`CWA_API_KEY`：只存後端本機忽略檔或 Coolify secret；有值時優先使用官方資料。
- `OPEN_METEO_FALLBACK_ENABLED`：官方來源無 key 或失敗時使用明確標示的模型資料；公開商業產品須先確認供應商方案與 attribution。
...
`database/migrations/001-operational-data.sql` 建立公開環境快取與匿名技術事件；`002-accounts.sql` 建立最小化帳號與 session table；`003-account-cloud-states.sql` 建立 cascade delete 的加密 state table。執行：
...
資料庫不保存完整活動文字、症狀、完整 prompt、context token、模型完整輸出、精確路線或原始 session token。啟用同步時，受 schema 限制的個人設定、粗略地點、日誌摘要與回饋僅以 AES-256-GCM ciphertext 保存。帳號資料僅有 Email、顯示名稱、password hash 與 session digest。詳見 [資料與儲存](../docs/data-and-storage.md)。

## 容器

`Dockerfile` 會建置 shared contracts 與 API，runtime 只安裝 contracts／backend production dependencies，以 `node` 非 root 使用者執行；container command 先跑 migration 再啟動 Fastify。Coolify 使用獨立 `airme-api` Dockerfile Application，Base Directory 是 `/`、Dockerfile Location 是 `/backend/Dockerfile`、port 是 `3000`，runtime 的 `DATABASE_URL` 使用 `airme-postgres` Resource 提供的 private URL。Coolify 的 HTTP healthcheck 使用 `GET http://127.0.0.1:3000/api/health`，Start Period 必須設為 90 秒，避免 migration 尚未完成就被回滾。根目錄 [docker-compose.yml](../docker-compose.yml) 只供本機三服務驗證；部署程序見 [部署計畫](../docs/deployment.md)。
